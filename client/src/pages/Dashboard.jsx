...
export default function Dashboard() {
  ...
  const fetchAll = async () => {
    ...
    setError(null)
    try {
      const [txRes, posRes, wlRes] = await Promise.all([
      ...
      // Fetch screener data — load user's saved tickers, then POST to screener
      try {
        let tickerList = null
        try {
          const tickerRes = await get('/api/screener/tickers')
          if (tickerRes.data?.length > 0) tickerList = tickerRes.data
        } catch {}
        const scrRes = await api.post('/api/screener', { tickers: tickerList || ['AAPL','MSFT','GOOGL','AMZN','TSLA','NVDA','META','BRK-B','JPM','V','WMT','JNJ','PG','KO','DIS'] })
        setScreenerData(scrRes.data?.stocks || scrRes.data?.data?.stocks || [])
      } catch { setScreenerData([]) }

      // Fetch plan
      try {
        const planRes = await get('/api/plan')
