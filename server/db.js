...
// Helper methods that mirror better-sqlite3 API but async
const db = {
...
async function initializeDatabase() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      has_viewed_dcf INTEGER DEFAULT 0,
      first_dcf_date TIMESTAMP,
      has_bought_undervalued INTEGER DEFAULT 0,
      ...
      spend_pct INTEGER DEFAULT 60,
      invest_pct INTEGER DEFAULT 20,
      monthly_income REAL DEFAULT 0,
      emergency_fund_complete INTEGER DEFAULT 0
    );
  ...
  await pool.query(`
    CREATE UNIQUE INDEX IF NOT EXISTS watchlist_user_ticker ON watchlist(user_id, ticker);
    CREATE UNIQUE INDEX IF NOT EXISTS budget_goals_user_cat ON budget_goals(user_id, category);
  `);
