...
router.get('/', async (req, res) => {
  try {
    const debts = await db.all('SELECT * FROM debts WHERE user_id = $1 ORDER BY interest_rate DESC', [req.userId]);
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await db.get(
      'INSERT INTO debts (user_id, name, balance, interest_rate, min_payment, original_amount) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
      [req.userId, name, balance, interest_rate, min_payment, balance]
    );
...
router.delete('/:id', async (req, res) => {
  try {
    await db.run('DELETE FROM debts WHERE id = $1 AND user_id = $2', [req.params.id, req.userId]);
...
router.post('/:id/payment', async (req, res) => {
  try {
    const debt = await db.get('SELECT * FROM debts WHERE id = $1 AND user_id = $2', [req.params.id, req.userId]);
    ...
    await db.run('UPDATE debts SET balance = $1 WHERE id = $2', [newBalance, debt.id]);
...
router.post('/plan', async (req, res) => {
  try {
    ...
    const debtState = sorted.map(d => ({
      id: d.id, name: d.name, originalBalance: d.balance, balance: d.balance,
      rate: d.interest_rate / 100 / 12, minPayment: d.min_payment,
      totalInterest: 0, monthsToPayoff: 0, monthlyPayment: d.min_payment, paidOff: false,
    }));
    ...
    while (debtState.some(d => !d.paidOff) && months < maxMonths) {
      ...
      for (const d of debtState) {
        ...
        if (d.balance <= 0.01) { d.paidOff = true; d.monthsToPayoff = months; d.balance = 0; extraLeft += d.minPayment; }
      }
    ...
    debtState.forEach(d => { if (!d.paidOff) d.monthsToPayoff = maxMonths; });
    ...
    let monthsNoExtra = 0;
    if (extraMonthly > 0) {
      const noExtraState = sorted.map(d => ({ balance: d.balance, rate: d.interest_rate / 100 / 12, minPayment: d.min_payment, paidOff: false }));
    ...
    res.json({
      ...
      data: {
        debts: debtState.map((d, i) => ({
          id: d.id, order: i + 1, name: d.name, originalBalance: d.originalBalance,
          interestRate: (d.rate * 12 * 100).toFixed(1), totalInterest: d.totalInterest,
          monthsToPayoff: d.monthsToPayoff, monthlyPayment: d.minPayment + (i === 0 ? extraMonthly : 0),
        })),
        totalDebt, totalInterest, debtFreeDate: debtFreeDate.toISOString(),
        monthsToFreedom: maxMonthsToPayoff, interestSaved, monthsSaved, strategy, aiSummary,
      }
    });
  } catch (err) {
    sendError(res, err);
