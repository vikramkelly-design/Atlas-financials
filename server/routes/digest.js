...
router.get('/', async (req, res) => {
  try {
    ...
    const portfolios = await db.all('SELECT id FROM portfolios WHERE user_id = $1', [userId]);
    let holdingsCount = 0;
    for (const p of portfolios) {
      const count = await db.get('SELECT COUNT(*) as cnt FROM portfolio_positions WHERE portfolio_id = $1', [p.id]);
      holdingsCount += parseInt(count?.cnt) || 0;
    }

    const budgetGoals = await db.all('SELECT * FROM budget_goals WHERE user_id = $1', [userId]);
    const transactions = await db.all('SELECT * FROM transactions WHERE user_id = $1 AND month = $2', [userId, currentMonth]);

    const totalBudget = budgetGoals.reduce((sum, g) => sum + parseFloat(g.monthly_limit), 0);
    const totalSpent = transactions.filter(t => t.amount < 0).reduce((sum, t) => sum + Math.abs(t.amount), 0);
    const budgetStatus = totalBudget > 0 ? (totalSpent <= totalBudget ? 'on_track' : 'over_budget') : 'no_budget';
    const budgetPct = totalBudget > 0 ? Math.round((totalSpent / totalBudget) * 100) : 0;

    const plan = await db.get('SELECT * FROM plans WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1', [userId]);
    ...
    res.json({
      ...
      data: {
        ...
        budgetStatus, budgetPct, totalSpent, totalBudget,
        plan: plan ? { goalAmount: plan.goal_amount, targetAge: plan.target_age, monthlyInvestment: plan.monthly_investment } : null,
        weeklyConcept,
      }
    });
