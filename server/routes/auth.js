...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = { id: result.id, name: name.trim(), email: email.toLowerCase().trim() };
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    const user = await db.get('SELECT id, email FROM users WHERE email = $1', [email.toLowerCase().trim()]);
    if (!user) {
    ...
    await db.run('INSERT INTO password_reset_tokens (user_id, token, expires_at) VALUES ($1, $2, $3)', [user.id, token, expiresAt]);
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    const reset = await db.get('SELECT * FROM password_reset_tokens WHERE token = $1', [token]);
    if (!reset) return res.status(400).json({ success: false, error: 'Invalid or expired reset link' });

    const now = new Date();
    if (now > new Date(reset.expires_at)) {
      await db.run('DELETE FROM password_reset_tokens WHERE id = $1', [reset.id]);
      return res.status(400).json({ success: false, error: 'Reset link has expired. Please request a new one.' });
    }

    const hash = await bcrypt.hash(password, 10);
    await db.run('UPDATE users SET password = $1 WHERE id = $2', [hash, reset.user_id]);
    await db.run('DELETE FROM password_reset_tokens WHERE id = $1', [reset.id]);
