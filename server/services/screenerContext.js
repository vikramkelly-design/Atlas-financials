...
async function buildScreenerContext(userId) {
  ...
  // Watchlist
  const watchlist = await db.all('SELECT ticker FROM watchlist WHERE user_id = $1', [userId]);
  ...
  try {
    const cachedResults = [];
    const screenerTickers = await db.all('SELECT ticker FROM screener_tickers WHERE user_id = $1', [userId]);
